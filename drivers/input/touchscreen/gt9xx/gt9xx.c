...
#if GTP_DEBUG_ON
    static const int  key_codes[] = {KEY_BACK,KEY_HOMEPAGE, KEY_APPSELECT};//, KEY_HOMEPAGE, KEY_F1, KEY_F2, KEY_F3};
    static const char *key_names[] = { "Key_Back","KEY_HOMEPAGE", "KEY_APPSELECT"};//, "Key_Homepage", "KEY_F1", "KEY_F2", "KEY_F3"};
#endif
...
static void gtp_parse_dt(struct device *dev)
{
	...
	gtp_3v3_gpio = of_get_named_gpio(np, "goodix,3v3-gpio", 0);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif
	u8 buf[4] = {GTP_REG_CONFIG_DATA>> 8, GTP_REG_CONFIG_DATA& 0xff};
	...
	if(gtp_3v3_gpio>0){
		ret = gpio_request(gtp_3v3_gpio, "gtp-3v3_en");
		if (ret < 0) {
				GTP_ERROR("%s: request 3v3_en pin failed\n", __func__);
				return ret;
			}
		gpio_direction_output(gtp_3v3_gpio, 1);
	}
	...
	sprintf(tp_version, "[fw]0x%2.2X,0x%2.2X [ic]GT917D", version_info,buf[2]);
